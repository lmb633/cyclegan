...
def visualize(modela2b, modelb2a, dataloader, num=16):
    ...
    for _, data in enumerate(dataloader):
        ...
        for j, fake_img in enumerate(fake_b):
            ...
            save_img(recover_a[j].cpu().detach(), 'images/a{0}{1}_recover.jpg'.format(i,j))
            save_img(img_a[j].cpu().detach(), 'images/a{0}{1}_img.jpg'.format(i,j))
        for j, fake_img in enumerate(fake_a):
            save_img(fake_img.cpu().detach(), 'images/b{0}{1}_out.jpg'.format(i,j))
            save_img(recover_b[j].cpu().detach(), 'images/b{0}{1}_recover.jpg'.format(i,j))
            save_img(img_b[j].cpu().detach(), 'images/b{0}{1}_img.jpg'.format(i,j))
        i += 1
        if i > num:
            break
